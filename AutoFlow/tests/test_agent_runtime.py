...
def test_completed_status_is_not_published_before_final_report_is_ready():
    report_started = threading.Event()
    release_report = threading.Event()

    class SlowReportingPlanner(FakePlanner):
        def final_response(self, goal, observation, history, proposed):
            report_started.set()
            assert release_report.wait(timeout=2)
            return "最终证据答复"

    runtime = AgentRuntime(SlowReportingPlanner([
        {"summary": "结束", "action": "finish", "arguments": {"message": "正在执行：旧消息"}},
    ]), FakeToolbox())
    session_id = runtime.start("等待最终答复")["id"]

    assert report_started.wait(timeout=2)
    assert runtime.get(session_id)["status"] == "running"
    release_report.set()
    result = wait_for(runtime, session_id, "completed")
    assert result["message"] == "最终证据答复"
...
def test_headless_browser_tools_are_registered_and_keep_dangerous_clicks_scoped_to_goal():
    class FakeBrowser:
        closed = False

        def open(self, url):
            return {"ok": True, "headless": True, "url": url}

        def element_summary(self, element_id):
            return {"name": "提交订单"} if element_id == "b003" else None

        def close(self):
            self.closed = True

    browser = FakeBrowser()
    toolbox = ComputerToolbox(browser=browser)

    names = {item["name"] for item in toolbox.schema()}
    assert {"browser_open", "browser_observe", "browser_click", "browser_type", "browser_download"} <= names
    assert toolbox.execute("browser_open", {"url": "https://example.com"})["headless"] is True
    assert toolbox.risk_for("browser_click", {"element_id": "b003"}) == "high"
    assert toolbox.goal_authorizes("提交订单", "browser_click", {"element_id": "b003"}) is True
    assert toolbox.goal_authorizes("查看订单", "browser_click", {"element_id": "b003"}) is False
    toolbox.close_session()
    assert browser.closed is True


def test_runtime_releases_task_scoped_browser_resources():
    class CleanupToolbox(FakeToolbox):
        closed = False

        def close_session(self):
            self.closed = True

    toolbox = CleanupToolbox()
    runtime = AgentRuntime(FakePlanner([
        {"summary": "结束", "action": "finish", "arguments": {"message": "完成"}},
    ]), toolbox)

    wait_for(runtime, runtime.start("后台网页任务")["id"], "completed")
    deadline = time.monotonic() + 1
    while not toolbox.closed and time.monotonic() < deadline:
        time.sleep(0.01)

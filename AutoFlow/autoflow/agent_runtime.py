...
class AgentRuntime:
    ...
    def _run(self, session: AgentSession) -> None:
        ...
        try:
            ...
            closer = getattr(self.toolbox, "close_session", None)
            if callable(closer):
                try:
                    closer()
                except Exception:
    ...
    def _mark_silent_capabilities(self, observation: dict[str, Any]) -> dict[str, Any]:
        observation["execution_mode"] = "background_preferred"
        observation["web_execution_mode"] = "headless_preferred"
        observation["silent_route"] = "网页任务优先 browser_*；文件任务优先专用文件工具；桌面应用再使用后台 UI 控件，最后才短暂前台操作。"
        return observation

...
@app.command("amd-check")
def amd_check(
    output: str = typer.Option("", "--output", help="可选：把非敏感环境证据保存为 JSON"),
) -> None:
    """检查 AMD Radeon GPU、ROCm/HIP 和本地推理环境。"""
    report = collect_amd_environment()
    console.print_json(json.dumps(report, ensure_ascii=False))
    if output.strip():
        destination = write_amd_evidence(Path(output), report)
        console.print(f"[green]证据已保存：{destination}[/green]")
    if not report["ready_for_rocm_inference"]:
        console.print("[yellow]当前机器没有同时检测到 AMD Radeon GPU 与 ROCm；请在 Radeon Cloud 或受支持的 AMD 主机重新运行。[/yellow]")
        raise typer.Exit(2)

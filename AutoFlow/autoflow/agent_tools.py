...
class ComputerToolbox:
    ...
    SPECS = (
        ...
        ToolSpec("browser_open", "在无界面浏览器中打开网页，不抢占鼠标或桌面窗口", "low", {"url": "http 或 https 网页地址"}),
        ToolSpec("browser_observe", "读取后台网页的标题、正文和可交互控件", "low", {}),
        ToolSpec("browser_click", "点击 browser_observe 返回的网页控件", "medium", {"element_id": "例如 b012"}),
        ToolSpec("browser_type", "向后台网页输入控件填写文字", "medium", {"element_id": "例如 b004", "text": "要输入的文字", "replace": "是否替换原内容"}),
        ToolSpec("browser_select", "选择后台网页下拉框选项", "medium", {"element_id": "例如 b007", "option": "选项文字或值"}),
        ToolSpec("browser_press", "向后台网页发送按键", "medium", {"keys": "例如 Enter、Control+L"}),
        ToolSpec("browser_back", "让后台浏览器返回上一页", "low", {}),
        ToolSpec("browser_download", "点击网页控件并把下载保存到用户下载目录的 AutoFlow 文件夹", "medium", {"element_id": "例如 b015"}),
    ...
    def __init__(self, observer: WindowsObserver | None = None, resolver: AppResolver | None = None, browser: HeadlessBrowser | None = None) -> None:
    ...
    def risk_for(self, action: str, arguments: dict[str, Any], observation: dict[str, Any] | None = None) -> str:
        ...
        if action == "browser_click":
            match = self.browser.element_summary(str(arguments.get("element_id", "")))
            if match and self.DANGEROUS_WORDS.search(str(match.get("name", ""))):
                return "high"
        return spec.risk
    ...
    def close_session(self) -> None:
        """Release task-scoped resources, including the headless browser process."""
        self.browser.close()
    ...
    def _tool_browser_open(self, arguments: dict[str, Any]) -> dict[str, Any]:
        return self.browser.open(self._required(arguments, "url"))

    def _tool_browser_observe(self, _arguments: dict[str, Any]) -> dict[str, Any]:
        return self.browser.observe()

    def _tool_browser_click(self, arguments: dict[str, Any]) -> dict[str, Any]:
        return self.browser.click(self._required(arguments, "element_id"))

    def _tool_browser_type(self, arguments: dict[str, Any]) -> dict[str, Any]:
        return self.browser.type_text(
            self._required(arguments, "element_id"),
            str(arguments.get("text", "")),
            bool(arguments.get("replace", True)),
        )

    def _tool_browser_select(self, arguments: dict[str, Any]) -> dict[str, Any]:
        return self.browser.select(self._required(arguments, "element_id"), self._required(arguments, "option"))

    def _tool_browser_press(self, arguments: dict[str, Any]) -> dict[str, Any]:
        return self.browser.press(self._required(arguments, "keys"))

    def _tool_browser_back(self, _arguments: dict[str, Any]) -> dict[str, Any]:
        return self.browser.back()

    def _tool_browser_download(self, arguments: dict[str, Any]) -> dict[str, Any]:
        return self.browser.download(self._required(arguments, "element_id"))
